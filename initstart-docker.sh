...
# account => zach:meizhi2018
 ...
 mount -t cifs -o username=user1,password=PuqG-6YR //10.0.1.195/znas-2 /mnt
...
docker run --restart=unless-stopped --name zach-mz-mysql -itd -v /data2/mysql-data:/var/lib/mysql -e "MYSQL_ROOT_PASSWORD=11111111" -e "TZ=Asia/Shanghai"  -p 3309:3306 mysql:5.7
...
#  for x in `docker ps | awk '/park/{print $NF}'`; do IMG=`docker ps | grep $x | awk '{print $2}' | sed 's/reg.devmz.cn:5845/zach/g'` ;docker commit -a 'zach.wang' $x $IMG; docker save $IMG | gzip > $x.tar.gz; done
 ...
 # NAS 
 mount -t cifs -o username=user1,password=Pu7a86YR //10.0.1.195/znas-2 /195-nas
 mount -t cifs -o username=user1,password=Pu7a86YR //192.168.1.12/znas-2 /195-nas-1000m
 # 学校标注系统
 mount -t cifs -o username=user1,password=Pu7a86YR //192.168.1.12/znas-2/label-data /mnt/label-data


docker network create zach-cctv-face-subnet
docker run --network zach-cctv-face-subnet --network-alias zach-mz-cctv-news-detect-mysql --restart unless-stopped -d -p 3306:3306 --name zach-mz-cctv-news-detect-mysql -v /data/zach-mz-cctv-news-detect/mysql-data/mysql.conf.d:/etc/mysql/conf.d -v /data/zach-mz-cctv-news-detect/mysql-data/logs:/var/log/mysql -v /data/zach-mz-cctv-news-detect/mysql-data/data:/var/lib/mysql -e "TZ=Asia/Shanghai" -e MYSQL_ROOT_PASSWORD=zach123 -d mysql:5.7
docker run --network zach-cctv-face-subnet --network-alias zach-mz-cctv-news-detect-front --restart unless-stopped -d --name zach-mz-cctv-news-detect-front -p 10001-10002:80-81 -e "TZ=Asia/Shanghai" -v /data/zach-mz-cctv-news-detect/nginx-config:/etc/nginx/conf.d -v /data/zach-mz-cctv-news-detect/front-page:/data/web -v /data/zach-mz-cctv-news-detect/storage-data:/storage-data nginx:1.18-alpine
docker run --network zach-cctv-face-subnet --network-alias zach-mz-cctv-news-detect-redis --restart unless-stopped -d --name zach-mz-cctv-redis -p 6379:6379 -e "TZ=Asia/Shanghai" -v /data/redis-data/data:/data  -e "TZ=Asia/Shanghai"  redis:5.0.10
docker run --network zach-cctv-face-subnet --network-alias zach-cctv-news-face-detect-backend-v12 --restart unless-stopped -itd --name zach-cctv-news-face-detect-backend-v1.2 --gpus all -v /data/zach-mz-cctv-news-detect/storage-data:/storage-data -v /data/zach-mz-cctv-news-detect/code/cctv-news-detect-backend:/data/code -p 10080:8000 -v /etc/localtime:/etc/localtime:ro -v /etc/timezone:/etc/timezone:ro zach-cctv-news-face-detect-base:v1.2

mount --bind /data2/storage-data-v3 storage-data-v3