...
# API Gateway handler
# if `redirect` is true, return 303 with Location set the the zip file
# otherwise return the signed url of the zip file
def api_handler(event, context):
    ...
    oss_client = get_oss_client(body, context)
    ret = _main_handler(oss_client, body, context)

    if evt['queryParameters']['redirect'] != 'false':
        return {
            'statusCode': 303,
            'headers': {
                'Location': sign_url(oss_client, ret, content_type='application/json')
            },
            'body': ''
        }
    else:
...
def main_handler(event, context):
    ...
    return sign_url(oss_client, ret)
